...
<div class="mb-3 row">
    ...
    <div class="col-md-9">
        <div class="input-group" id="datepicker1">
            <input type="text" class="form-control" placeholder="تاريخ بداية الحلقة"
                   name="start_date" value="{{old('start_date',$circle->start_date )}}" id="start_date"
    ...
    <div class="col-md-3">
        <select class="form-control" name="area_id">
            ...
            @foreach($areas as $key => $area)
                <option value="{{ $area->id }}" @if($area->id == $circle->area_father_id) selected @endif>{{ $area->name }}</option>
            @endforeach
        </select>
    </div>
        ...
        <select class="form-control" name="sub_area_id" id="sub_area_id">
            @if(isset($sub_areas))
                @foreach($sub_areas as $key => $area)
                    <option value="{{ $area->id }}" @if($area->id == $circle->area_id) selected @endif>{{ $area->name }}</option>
                @endforeach
            @endif
        </select>
    </div>
        ...
        <select class="form-control" name="place_id" id="place_id">
            @if(isset($places))
                @foreach($places as $key => $place)
                    <option value="{{ $place->id }}" @if($place->id == $circle->place_id) selected @endif>{{ $place->name }}</option>
                @endforeach
            @endif
        </select>
    </div>
...
<div class="mb-3 row">
    ...
    <div class="col-md-9">
        <select class="form-control" name="supervisor_id" id="supervisor_id">
            <option value="">-- تحديد --</option>
            @if(isset($supervisors)) {!! $supervisors !!} @endif
        </select>
    </div>
</div>
<div class="mb-3 row">
...
<script>
    ...
    $('#place_id').on('change', function() {
        ...
        $.get('/getPlaceTeachersForCircles/'+place_id+'/{{ $circle->teacher_id ? $circle->teacher_id : 0 }}',function(data){
            $('#teacher_id').empty().html(data);
        });
        $.get('/getPlaceAreaSupervisorForCircles/'+place_id+'/{{ $circle->supervisor_id ? $circle->supervisor_id : 0 }}',function(data){
