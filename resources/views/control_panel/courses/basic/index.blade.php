    ...
    <script>
        ...
        @if(hasPermissionHelper('فلترة الدورات العلمية'))
            ...
            function updateDateTable(){
                table.ajax.url(
                    "/getCoursesData?teacher_id="+$('#teachers_select').val()+"&book_id="+$('#books_select').val()+"&sub_area_id="+$('#sub_areas_select').val()+'&area_id='+$('#areas_select').val()+'&status='+$('#filterCoursesByStatus').val()+'&place_area='+$('#place_area').val()
                ).load();
            }
        @endif
