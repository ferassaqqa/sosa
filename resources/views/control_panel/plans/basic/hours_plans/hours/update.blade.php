...
<div class="modal-header">
    <h5 class="modal-title" id="myLargeModalLabel" style="color: #000 !important;"> تعديل ساعات ضمن خطة كتاب {{$book->name}} - {{ $plan_name }} -</h5>
    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
</div>
<div class="modal-body">
    <div class="row">
        <form action="{{ route('planHours.update',$plan->id) }}" method="POST" id="updateMonthForm">
            <input type="hidden" name="_method" value="PUT">
            @include('control_panel.plans.basic.hours_plans.hours.form')
        </form>
    </div>
</div>
