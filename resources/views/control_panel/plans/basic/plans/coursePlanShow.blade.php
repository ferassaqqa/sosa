...
<div class="modal-header">
    <h5 class="modal-title" id="myLargeModalLabel" style="color: #000 !important;"> تعديل بيانات {{$department_name}} للعام {{ $year }}  </h5>
                    ...
                    @if(isset($books))
                        @foreach($books as $key => $book)
                                @if(!$key)
                                    <script>
                                        ...
                                        $('#area_{{ $area->id }}').text('{{ $area->getAreaNameWithPercentage($year,$book->id) }}');
                                    </script>
                                @endif
                            <tr>
                                ...
                                @foreach($area->subArea as $key => $subArea)
                                    <script>
                                        {{--console.log('area_{{ $area->id }}');--}}
                                        $('#area_{{ $subArea->id }}').text('{{ $subArea->getAreaNameWithPercentage($year,$book->id) }}');
                                    </script>
                                    <td>
                        ...
                        @endforeach
                            <tr>
                                ...
                                @foreach($area->subArea as $key => $subArea)
                                    <td>
                                        <input disabled class="form-control subArea_{{$subArea->id}}" value="{{getCoursePlanSubAreaBookValue($subArea->id, null ,$year )}}" oninput="checkTotalLimit(this,'{{ $subArea->id }}')" type="number" min="0" step="1" name="sub_area_value[{{$area->id}}][{{$subArea->id}}][{{$book->id}}]" style="width: 92px;margin: 0px auto;">
                                    </td>
                                @endforeach
                            </tr>
                    @endif
