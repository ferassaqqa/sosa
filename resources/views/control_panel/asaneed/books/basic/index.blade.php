...
@endsection
@section('title')
    <title>برنامج السنة | كتب الأسانيد والاجازات</title>
@endsection
@section('content')
...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0"> كتب الأسانيد والاجازات</h4>

            <div class="page-title-right">
                <ol class="breadcrumb m-0">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard.index') }}">الرئيسية</a></li>
                    <li class="breadcrumb-item active"> كتب الأسانيد والاجازات</li>
                </ol>
            </div>
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                <div class="row mb-3">
                    <div class="col-md-3">
                        <button class="btn btn-primary call-user-modal" data-url="{{route('asaneedBooks.create')}}" data-bs-toggle="modal" data-bs-target=".bs-example-modal-xl" style="width: 305px;">
                    ...
                    <div class="col-md-3" style="margin-right: -11px;">
                        <select class="form-control" onchange="updateDateTable()" id="authorSelect">
                            ...
                            @foreach($asaneedBookAuthors as $asaneedBookAuthor)
                                <option value="{{ $asaneedBookAuthor }}">{{ $asaneedBookAuthor }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
                        <select class="form-control" onchange="updateDateTable()" id="courseBookCategories">
                            ...
                            @foreach($asaneedBookCategories as $asaneedBookCategory)
                                <option value="{{ $asaneedBookCategory->id }}">{{ $asaneedBookCategory->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            table = $('#dataTable').DataTable( {
                "processing": true,
                "serverSide": true,
                "ajax": "{{ route('asaneedBooks.getData') }}",
                language: {
                    search: "بحث",
        ...
        function copyToYear(obj,book_id) {
            ...
            obj.disabled= true;
            $.get('/getYearsDoesNotHaveThisAsaneedBook/'+book_id,function (data) {
                ...
                Swal.fire({
                    ...
                    preConfirm: function(value) {
                        ...
                        if (year == 0) {
                            ...
                            return fetch('/copyAsaneedBookDetailsToYear/' + year + '/' + book_id)
        ...
        function updateDateTable() {
            table.ajax.url(
                "/getAsaneedBooksData/?courseBookCategory="+$('#courseBookCategories').val()+'&author='+$('#authorSelect').val()
            ).load();
        }
