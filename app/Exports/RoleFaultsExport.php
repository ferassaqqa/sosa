...
class RoleFaultsExport implements FromCollection,WithHeadings,WithStyles,ShouldAutoSize,WithEvents
{
    ...
    */
    public function collection()
    {
        ...
        foreach(Self::$faults as $key => $failure){
            $collection[$key] = ['#'=>$failure->row(),'name'=>$failure->values()[$failure->attribute()]];
        }
        return $collection;
    }
    ...
    {
        foreach(Self::$faults as $key => $failure){
            $role = Role::onlyTrashed()->where('name',$failure->values()[$failure->attribute()])->first();
            try {
                if($role) {
                    $event->sheet
                        ->getDelegate()
